...
.control
  ...
  set wr_singlescale
  tran 0.01n 200n
  wrdata ~/documents/git/madvlsi/csrl/schematic/csrl_tran.txt v(clk) v(D) v(Dn) v(Q0) v(Qn0) v(Q1) v(Qn1) v(Q2) v(Qn2) v(Q3) v(Qn3)
.endc"}
C {sky130_fd_pr/corner.sym} 350 -280 0 0 {name=CORNER only_toplevel=false corner=tt}
...
value="pwl(0 0 15ns 0 16ns 1.8 30ns 1.8 31n 0)"
}
...
value="pwl(0 1.8 15ns 1.8 16ns 0 30ns 0 31ns 1.8)"
}
...
value="pwl(0 1.8 25ns 1.8 26ns 0 40ns 0 41ns 1.8)"}
C {csrl_falling_edge.sym} 410 -410 0 0 {name=X1}
C {csrl_falling_edge.sym} 510 -410 0 0 {name=X2}
C {csrl_falling_edge.sym} 610 -410 0 0 {name=X3}
C {csrl_falling_edge.sym} 710 -410 0 0 {name=X4}
